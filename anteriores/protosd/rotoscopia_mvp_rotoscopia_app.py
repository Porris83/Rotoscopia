"""
Rotoscopia MVP - rotoscopia_app.py

Prototipo mínimo para rotoscopiar manualmente un video frame a frame.

Requisitos:
  pip install PySide6 opencv-python pillow numpy

Limitaciones:
 - Este prototipo carga todos los frames en memoria (útil para clips cortos).
 - Pensado como punto de partida: features por agregar: zoom/pan, onion-skin, brush sizes, suavizado, export sprite sheet, etc.
...
class DrawingCanvas(QtWidgets.QLabel):
    ...
    def update_display(self, background_pixmap: QtGui.QPixmap = None, opacity: float = 0.5):
    ...
    def get_overlay_image(self) -> QtGui.QImage:
        if self.overlay is None:
            return None
        return self.overlay.toImage()

    def set_overlay_from_image(self, qimg: QtGui.QImage):
        if qimg is None:
            return
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def load_frames(self, path):
        ...
        cap.release()
        self.frame_label.setText(f"Frame: 0 / {len(self.frames)-1}")
    ...
    def refresh_view(self):
        ...
        self.frame_label.setText(f"Frame: {idx} / {len(self.frames)-1}")
